...
CREATE INDEX idx_movie_platforms_movie_id ON movie_platforms(movie_id);
CREATE INDEX idx_movie_platforms_platform_id ON movie_platforms(platform_id);
...
CREATE POLICY "Admin manage platforms" ON platforms FOR ALL USING ((auth.jwt() -> 'user_metadata' ->> 'role') = 'admin');
CREATE POLICY "Admin manage movie_platforms" ON movie_platforms FOR ALL USING ((auth.jwt() -> 'user_metadata' ->> 'role') = 'admin');
