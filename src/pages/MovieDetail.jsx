          ...
          {movie.genres && movie.genres.length > 0 && (
            <div className="flex gap-2">
              {movie.genres.slice(0, 3).map((genre, i) => (
                <button key={i} onClick={() => navigate(`/movies?genre=${genre}`)} className="text-gray-300 hover:text-red-500 transition-colors">
                  {genre}
                </button>
              ))}
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-8">
        <div className="border-b border-white/10 mb-6">
          <div className="flex gap-6">
            ...
            <button onClick={() => setActiveTab('links')} className={`pb-2 ${activeTab === 'links' ? 'border-b-2 border-red-600 text-red-600' : 'text-gray-400'}`}>
              Links
            </button>
            <button onClick={() => setActiveTab('crew')} className={`pb-2 ${activeTab === 'crew' ? 'border-b-2 border-red-600 text-red-600' : 'text-gray-400'}`}>
              Crew
            </button>
          </div>
        </div>
          ...
          <div className="space-y-8">
            ...
            {movie.trailer_url && (
              <div>
                ...
                <a href={movie.trailer_url} target="_blank" rel="noopener noreferrer" className="block relative group">
                  <img src={getYouTubeThumbnail(movie.trailer_url) || movie.backdrop_url || 'https://via.placeholder.com/640x360'} alt="Trailer" className="w-full md:w-2/3 rounded-lg" />
            ...
            {movie.telegram_link && (
              <div>
                ...
                <a href={movie.telegram_link} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 bg-blue-500 hover:bg-blue-600 rounded-lg transition font-semibold">
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-3">
                  {movie.music_links.spotify && (
                    <a href={movie.music_links.spotify} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 bg-green-600 hover:bg-green-700 rounded-lg transition">
                      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                  ...
                  {movie.music_links.apple_music && (
                    <a href={movie.music_links.apple_music} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 bg-pink-600 hover:bg-pink-700 rounded-lg transition">
                      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
                  ...
                  {movie.music_links.youtube_music && (
                    <a href={movie.music_links.youtube_music} target="_blank" rel="noopener noreferrer" className="flex items-center gap-2 px-4 py-2 bg-red-600 hover:bg-red-700 rounded-lg transition">
                      <svg className="w-5 h-5" viewBox="0 0 24 24" fill="currentColor">
        ...
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50" onClick={() => setEditingPoster(false)}>
          <div className="bg-gray-900 p-6 rounded-xl max-w-md w-full mx-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={async () => { await updateMovie(movie.id, { poster_url: posterUrl }); setEditingPoster(false); loadMovie(); }} className="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 rounded-lg">
                Save
              </button>
      ...
      {editingBackdrop && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50" onClick={() => setEditingBackdrop(false)}>
          <div className="bg-gray-900 p-6 rounded-xl max-w-md w-full mx-4" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex gap-3">
              <button onClick={async () => { await updateMovie(movie.id, { backdrop_url: backdropUrl }); setEditingBackdrop(false); loadMovie(); }} className="flex-1 px-4 py-2 bg-red-600 hover:bg-red-700 rounded-lg">
                Save
              </button>
